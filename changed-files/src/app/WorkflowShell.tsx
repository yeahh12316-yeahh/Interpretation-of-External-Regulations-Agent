...
import type { AnalysisCheckpoint } from "../features/analysis/skill-orchestrator";
import {
  deleteAnalysisCheckpoint,
  loadAnalysisCheckpoint,
  saveAnalysisCheckpoint,
} from "../features/analysis/checkpoint-store";
import {
...
import {
  ...
  saveSessionFile,
  setRawFileRetention,
} from "../features/projects/session-files";
...
export interface WorkflowShellProps {
  ...
  /** When provided, renders a header entry point for the project manager. */
...
export function WorkflowShell({
  ...
  const [retainedSourceIds, setRetainedSourceIds] = useState<
    ReadonlySet<string>
  >(new Set());
  const [resumableCheckpoint, setResumableCheckpoint] =
    useState<AnalysisCheckpoint | null>(null);
  ...
  useEffect(() => {
    void getRawFileRetention()
      .then(setRawRetention)
      .catch(() => undefined);
  }, []);
  useEffect(() => {
    let active = true;
    void listSessionFiles(session.project.projectId)
      .then((records) => {
        if (active) {
          setRetainedSourceIds(
            new Set(records.map(({ sourceId }) => sourceId)),
          );
        }
      })
      .catch(() => undefined);
  ...
  useEffect(() => {
    let active = true;
    void loadAnalysisCheckpoint(session.project.projectId)
      .then((checkpoint) => {
        if (active) setResumableCheckpoint(checkpoint);
      })
      .catch(() => undefined);
    return () => {
      active = false;
  ...
  const handleParsed = (result: ParseResult, file?: File) => {
    ...
    persist(candidate);
    if (file && rawRetention) {
      void retainRawFile(result, file);
    }
  };
  const retainRawFile = async (result: ParseResult, file: File) => {
    const sourceId = result.source.sourceId;
    try {
      const bytes = await file.arrayBuffer();
      await saveSessionFile({
        sourceId,
        projectId: sessionRef.current.project.projectId,
      ...
      setRetainedSourceIds((current) => new Set(current).add(sourceId));
    } catch {
      setMessage({
        kind: "error",
        text: "原始文件本机保留失败，解析结果不受影响",
      });
    }
  };
  const toggleRawRetention = (enabled: boolean) => {
    setRawRetention(enabled);
    void setRawFileRetention(enabled).catch(() =>
      setMessage({ kind: "error", text: "本机保留偏好保存失败" }),
    );
  };
  ...
  const executeAnalysis = async (resumeFrom?: AnalysisCheckpoint) => {
    ...
    if (!resumeFrom) {
      // A fresh run discards any previous resume point so it cannot be
      // resumed into after this run starts emitting its own checkpoints.
      void deleteAnalysisCheckpoint(startingSession.project.projectId);
    ...
    try {
      ...
      const draft = await runAnalysisImpl(
        ...
        (update: AnalysisProgress) => {
          ...
          const projectId = startingSession.project.projectId;
          setResumableCheckpoint(update.checkpoint);
          void saveAnalysisCheckpoint(projectId, update.checkpoint).catch(
            () => undefined,
          );
        },
      );
  ...
  const applyDraft = (draft: AnalysisDraft, current: WorkflowSession) => {
    if (current.pendingReanalysis) {
      ...
      finalizeAnalysisRun(current.project.projectId);
  ...
  const finalizeAnalysisRun = (projectId: string) => {
    void deleteAnalysisCheckpoint(projectId);
    setResumableCheckpoint(null);
  };
  ...
  const page = draftReportOpen ? (
    ...
    <section>
      ...
      <MaterialUpload
        ...
        retentionEnabled={rawRetention}
        onRetentionChange={toggleRawRetention}
      />
      {session.project.sourceUnits.length > 0 ? (
        <ul data-testid="retained-source-files">
          {session.project.sourceUnits.map((source) => (
    ...
    <>
      <AnalysisPage
        ...
        progress={progress}
      />
      {resumableCheckpoint && !running ? (
        <div role="status" data-testid="analysis-resume-banner">
          <p>
            检测到未完成的分析：已完成 {resumableCheckpoint.runs.length} 个节点，可从断点继续。
          </p>
          <button
            type="button"
            onClick={() => void executeAnalysis(resumableCheckpoint)}
          >
            从断点继续分析
          </button>
          <button
            type="button"
            onClick={() => {
              void deleteAnalysisCheckpoint(session.project.projectId);
              setResumableCheckpoint(null);
              setMessage({ kind: "status", text: "已放弃分析断点" });
            }}
          >
            放弃断点
          </button>
