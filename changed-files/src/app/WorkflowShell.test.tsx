...
it("retains raw source files locally only after the retention switch is enabled", async () => {
  const user = userEvent.setup();
  const content = "第一条 商业银行应当审慎经营。";
  const bytes = new TextEncoder().encode(content);
  const digest = await crypto.subtle.digest("SHA-256", bytes);
  const fileHash = Array.from(new Uint8Array(digest), (byte) =>
    byte.toString(16).padStart(2, "0"),
  ).join("");
  const sourceId = "SRC-regulatory_text-" + fileHash.slice(0, 20);
  const parseFile = vi.fn(
    async (
      ...
      const source = {
        sourceId: "SRC-" + sourceType + "-" + fileHash.slice(0, 20),
        sourceType,
        title: "regulation.txt",
        content,
      };
      const unit = {
        unitId: "U1",
  ...
  render(
    <WorkflowShell initialSession={emptySession()} parseFile={parseFile} />,
  );

  const upload = () =>
    new File([bytes], "regulation.txt", { type: "text/plain" });

  // Retention defaults to off: parsing works but nothing is retained.
  await user.upload(screen.getByLabelText("选择监管文件"), upload());
  expect(await screen.findByText("解析完成", { selector: "p" })).toBeVisible();
  expect(await projectDatabase.sessionFiles.count()).toBe(0);
  expect(screen.getByTestId("retained-source-files")).toHaveTextContent(
    "原始文件未在本机保留",
  );

  // Enabling the switch persists the preference and the next upload.
  await user.click(screen.getByLabelText("保留原始文件副本"));
  await user.upload(screen.getByLabelText("选择监管文件"), upload());
  expect(await screen.findByText("解析完成", { selector: "p" })).toBeVisible();
  const retained = await projectDatabase.sessionFiles.toArray();
...
it("resumes an interrupted analysis from the persisted checkpoint", async () => {
  const user = userEvent.setup();
  sessionCredentials.set({
    baseUrl: "https://model.example/v1",
    apiKey: "session-only-secret",
  ...
  const persistedCheckpoint = {
    ...
    runs: [
      {
        nodeId: "document_identity",
        stage: "document_identity",
        chunkId: "chunk-0",
        model: "safe-model",
        promptVersion: "v1",
        inputSourceIds: ["REG-A"],
    ...
    limitations: [],
    lastSuccessfulNode: "document_identity",
  };
  ...
  const run = vi.fn(
    (
      input: { resumeFrom?: unknown },
      signal: AbortSignal,
      onProgress?: (update: { checkpoint: unknown }) => void | Promise<void>,
    ) => {
      callCount += 1;
      if (callCount === 1) {
        // First run: synchronously report progress, then hang until aborted.
        // Synchronous reporting guarantees the resume checkpoint is set
        // before the test clicks "cancel".
        void onProgress?.({ checkpoint: persistedCheckpoint });
      ...
      expect(input.resumeFrom).toMatchObject({
        lastSuccessfulNode: "document_identity",
      });
      return Promise.resolve(completedDraft);
    },
  );
  const repository = {
    load: vi.fn(),
  ...
  // First run: start, let the progress callback persist a checkpoint,
  // then unmount (simulates a refresh mid-flight). Remount loads the
  // resume point via the effect and shows the banner.
  const first = render(
    <WorkflowShell
      initialSession={analysisSession()}
      repository={repository}
  ...
  await user.click(screen.getByRole("button", { name: "开始监管分析" }));
  await waitFor(() =>
    expect(projectDatabase.analysisCheckpoints.count()).resolves.toBe(1),
  );
  const stored = await projectDatabase.analysisCheckpoints.get(
    analysisSession().project.projectId,
  );
  expect(
    (stored?.checkpoint as { runs?: unknown[] })?.runs?.length,
  ).toBe(1);
  first.unmount();
  ...
  expect(
    await screen.findByTestId("analysis-resume-banner"),
  ).toHaveTextContent("检测到未完成的分析");

  // Resume the second run from the persisted checkpoint.
  await user.click(screen.getByRole("button", { name: "从断点继续分析" }));
  await waitFor(() =>
    expect(run.mock.calls[1][0].resumeFrom).toMatchObject({
      lastSuccessfulNode: "document_identity",
    }),
  );
  // The resume point stays available until a run completes successfully;
  // a failed resume keeps it so the user can retry from the same point.
  expect(
    await projectDatabase.analysisCheckpoints.count(),
  ).toBe(1);
});
